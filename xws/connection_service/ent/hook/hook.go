...
// The BlockedUserFunc type is an adapter to allow the use of ordinary
// function as BlockedUser mutator.
type BlockedUserFunc func(context.Context, *ent.BlockedUserMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f BlockedUserFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.BlockedUserMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.BlockedUserMutation", m)
	}
	return f(ctx, mv)
}
