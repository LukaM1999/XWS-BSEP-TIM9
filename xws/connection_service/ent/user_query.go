...
func (uq *UserQuery) QueryConnection() *UserQuery {
...
func (uq *UserQuery) QueryBlock() *BlockedUserQuery {
	query := &BlockedUserQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryConnections() *ConnectionQuery {
	query := &ConnectionQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withConnection:  uq.withConnection.Clone(),
		withBlock:       uq.withBlock.Clone(),
...
func (uq *UserQuery) WithConnection(opts ...func(*UserQuery)) *UserQuery {
	query := &UserQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withConnection = query
	return uq
}
...
func (uq *UserQuery) WithBlock(opts ...func(*BlockedUserQuery)) *UserQuery {
	query := &BlockedUserQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithConnections(opts ...func(*ConnectionQuery)) *UserQuery {
	query := &ConnectionQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withConnection; query != nil {
		...
		for i, node := range nodes {
			edgeids[i] = node.ID
			byid[node.ID] = node
			node.Edges.Connection = []*User{}
		}
		query.Where(func(s *sql.Selector) {
			joinT := sql.Table(user.ConnectionTable)
			s.Join(joinT).On(s.C(user.FieldID), joinT.C(user.ConnectionPrimaryKey[1]))
			s.Where(sql.InValues(joinT.C(user.ConnectionPrimaryKey[0]), edgeids...))
			columns := s.SelectedColumns()
			s.Select(joinT.C(user.ConnectionPrimaryKey[0]))
		...
		for _, n := range neighbors {
			nodes, ok := nids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "connection" node returned %v`, n.ID)
			}
			for kn := range nodes {
				kn.Edges.Connection = append(kn.Edges.Connection, n)
			}
		}
	}

	if query := uq.withBlock; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Block = []*BlockedUser{}
		}
		query.withFKs = true
		query.Where(predicate.BlockedUser(func(s *sql.Selector) {
			s.Where(sql.InValues(user.BlockColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_block
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_block" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_block" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Block = append(node.Edges.Block, n)
		}
	}
	...
	if query := uq.withConnections; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Connections = []*Connection{}
		}
		query.Where(predicate.Connection(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ConnectionsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.Connections = append(node.Edges.Connections, n)
		}
	}
