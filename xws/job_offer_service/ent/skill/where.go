...
// HasRequired applies the HasEdge predicate on the "required" edge.
func HasRequired() predicate.Skill {
	return predicate.Skill(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(RequiredTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, RequiredTable, RequiredPrimaryKey...),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasRequiredWith applies the HasEdge predicate on the "required" edge with a given conditions (other predicates).
func HasRequiredWith(preds ...predicate.JobOffer) predicate.Skill {
	return predicate.Skill(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(RequiredInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, RequiredTable, RequiredPrimaryKey...),
		)
