...
func (store *ReactionMongoDBStore) Get(postId string) ([]*domain.Reaction, error) {
...
func (store *ReactionMongoDBStore) Reaction(reaction *domain.Reaction) (*domain.Reaction, error) {
	...
	if len(existingReaction) > 0 {
		result, err2 := store.UpdateReaction(reaction, filter)
		if err2 != nil {
			return nil, err2
		}
		return result, nil
...
func (store *ReactionMongoDBStore) UpdateReaction(reaction *domain.Reaction, filter bson.M) (updatedReaction *domain.Reaction, err error) {
	update := bson.M{"$set": bson.M{"createdAt": reaction.CreatedAt, "type": reaction.Type}}
	after := options.After
	opt := options.FindOneAndUpdateOptions{
		ReturnDocument: &after,
	}
	result := store.reactions.FindOneAndUpdate(context.TODO(), filter, update, &opt)
	if result.Err() != nil {
...
func (store *ReactionMongoDBStore) Delete(id string) error {
	reactionId, err := primitive.ObjectIDFromHex(id)
	if err != nil {
		return err
	}
	_, err = store.reactions.DeleteOne(context.TODO(), bson.M{"_id": reactionId})
	if err != nil {
		return err
	}
	return nil
